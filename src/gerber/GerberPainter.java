...
public class GerberPainter implements IGerberPainter {
	protected int ppi;
	...
	public GerberPainter(String filename,int ppi, boolean mirroVertical, boolean mirroHorizontal) {
		this.filename = filename;
		this.ppi = ppi;
		this.border = (int)Math.round(ConfigTool.getInstance().border*ppi);
	...
	protected int convertY(int y, int height) {
		...
		if(mirroVertical)
			ret = this.clipHeight+border*2-(border+y+height- clipY+moveY);
		return ret;
	}
	...
	protected int convertX(int x, int width) {
		...
		if(mirroHorizontal)
			ret = this.clipWidth+border*2-(border+x+width - clipX + moveX);
		return ret;
	}
...
//		System.out.println("" + new Rectangle(x1,y1,x2,y2) + new Rectangle(x3,y3,x4,y4));
